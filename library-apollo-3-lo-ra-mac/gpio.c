...
//IRQ needs to be set up still
const am_hal_gpio_pincfg_t gpioPullIntPAR =
{
...
uint32_t initGPIO(void){
	...
	success += am_hal_gpio_pinconfig(LORA_IRQ,g_AM_HAL_GPIO_INPUT);			//used for LoRa
	success += am_hal_gpio_pinconfig(AS7341_INT_PIN,gpioPullIntPAR);			  //Used as Interrupt pin for PAR measurement
