...
#define SUBSECONDS_IN_1SECONDS						100
...
#define MIN_ALARM_DELAY 							1		//the alarm setting routine required 14�s
...
void RtcInit( void ){
	if(RtcInitialized == false){
		...
		am_hal_rtc_alarm_set(&currentTime,AM_HAL_RTC_ALM_RPT_DIS);		//reset alarmtime
...
void RtcSetAlarm( uint32_t timeout ){
	...
	RtcStartAlarm(timeout);		//currently this function has no real application other than to supply the framework to be happy
}
...
void RtcStartAlarm( uint32_t timeout ){
	...
	rtcAlarmSubSeconds += RtcTimerContext.CalenderTime.ui32Hundredths;
	...
	while( rtcAlarmSubSeconds >= SUBSECONDS_IN_1SECONDS)
	{
		rtcAlarmSubSeconds -= SUBSECONDS_IN_1SECONDS;
		rtcAlarmSeconds++;
	}
